...
void XMLCALL ChapterHtmlSlimParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (matches(name, HEADER_TAGS, std::size(HEADER_TAGS))) {
    ...
    if (strcmp(name, "br") == 0) {
      ...
      const bool flowHasText = self->flowHasText;
      const bool flowHasCjk = self->flowHasCjk;
      if (flowHasText && flowHasCjk) {
        // The <br> sits inside CJK flowing text, where <br> is the book's paragraph
        // separator (common CJK formatting): plain block boundary with bottom margins
        // stripped. The scene-break gap would otherwise fire between every paragraph
        // and collapse page capacity. A standalone <br> between blocks still gets the
        // scene-break gap, so CJK books using <p> + <br> scene breaks keep them.
      ...
      self->flowHasText = flowHasText;
      self->flowHasCjk = flowHasCjk;
    } else {
      self->currentCssStyle = cssStyle;
